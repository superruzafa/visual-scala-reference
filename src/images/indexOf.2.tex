...
\node (B) [below right=3cm and 0cm of an.south east] {-1};
...
\foreach \i/\s in {
  ...
  n/0.5em}
{
  \node (pa\i) [predicate, align=center, below=\s of a\i, pin={[function result, pin position=below] \false}] {$a_{\i}$ = \param{elem}?};
  \draw (a\i) -- (pa\i);
}
