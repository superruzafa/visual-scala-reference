...
\draw [flow width] (f2.east io 1) -| ++(.25, 0.75) coordinate (flowy);
\draw [flow width, solid dashed solid] (flowy) -- (flowy -| fi.north io 1) coordinate (last);
\draw [flow] (last) -- (fi.north io 1);

\draw [flow width] (fi.east io 1) -| ++(.25, 0.75) coordinate (last);
